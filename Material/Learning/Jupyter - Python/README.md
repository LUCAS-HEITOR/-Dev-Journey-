<div align="center">

# 🐍 Python Interactive Notebooks

Hands-on Jupyter notebooks exploring Python fundamentals and language mechanics.
<br/><br/>

### 📓 Available Notebooks

- 🔹 [**`Basics.ipynb`**](Basics.ipynb) — Complete Python beginner guide (types, loops, lists, dicts, functions).
- 🔹 [**`Classes.ipynb`**](Classes.ipynb) — Object-Oriented Programming, `__init__`, `@property`, and inheritance.
- 🔹 [**`DataTypes.ipynb`**](DataTypes.ipynb) — Python Data Type system, mutability, numerics, sequences, sets, mappings, and Big-O complexity.
- 🔹 [**`Decorator.ipynb`**](Decorator.ipynb) — Function decorators, closures, and `@wraps`.
- 🔹 [**`Def.ipynb`**](Def.ipynb) — Function anatomy, parameters (`/`, `*`, `*args`, `**kwargs`), LEGB scopes, and closures.
- 🔹 [**`Returns.ipynb`**](Returns.ipynb) — Return mechanisms, `yield`, `yield from`, memory benchmarking, and async generators.
- 🔹 [**`Set.ipynb`**](Set.ipynb) — Mathematical set operations, hashability, and $O(1)$ lookups.
- 🔹 [**`Treatment.ipynb`**](Treatment.ipynb) — Complete Exception Handling & Resource Treatment (`try/except/else/finally`, custom errors).

<br/>

[⬅️ Back to Main Repository](../../../README.md)

</div>
